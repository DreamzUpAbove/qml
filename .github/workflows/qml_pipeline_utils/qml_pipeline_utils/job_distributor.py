...
For best result of distribution, sort the tasks from the highest load to lowest and pass each to
SortedWorkerHandler.add_task_to_workers. Alternatively, pass the entire list of tasks to SortedWorkerHandler.add_tasks_to_workers
and the function will sort them.
...
a sorted list of workers internally, after a task is added to a worker, the list is re-sorted such that the first element
is always the worker currently with the least amount of load.
...
The `load` here refers to the amount of time (in seconds) it takes to execute a sphinx demo.
